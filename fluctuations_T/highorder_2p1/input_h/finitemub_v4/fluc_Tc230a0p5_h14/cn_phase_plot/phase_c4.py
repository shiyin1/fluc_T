...
vd=0.001
vu=0.01
im=ax2.imshow(c4data.T, cmap=cmap_custom,interpolation='nearest',vmin=vd,vmax=vu,zorder=3,extent=[0, 555, 21, 300], origin='lower', aspect=1.7)
vnorm = mpl.colors.Normalize(vmin=vd, vmax=vu)
plt.rcParams['font.size'] = 7
cbar=plt.colorbar(im,fraction=0.029, pad=0.02,norm=vnorm)
cbar.set_label(r'$c_4$', rotation=0,fontsize=12,labelpad=10)
plt.scatter(620,99,color='red',marker='o',s=30,label=r'CEP',zorder=3)
pb1,=ax2.plot(pb[:,0],pb[:,1],dashes=[1,0],color='k',linewidth=1.2,alpha=0.6,label=r'Crossover',zorder=3)
...
fig.savefig("phasec4.pdf",dpi=300,transparent=True)