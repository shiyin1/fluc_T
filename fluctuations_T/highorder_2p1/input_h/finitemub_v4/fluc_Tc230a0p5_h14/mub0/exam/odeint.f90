...
!/path/ contains its own information about how often an intermediate value is to be stored.
  ...
  PARAMETER (MAXSTP=10000,NMAX=50,KMAXX=200,TINY=1.Q-30)
...
!User storage for intermediate results. Preset dxsav and kmax
  ...
  if (kmax.gt.0) xsav=x-2.Q+00*dxsav
  do nstp=1,MAXSTP
    ...
    if((x+h-x2)*(x+h-x1).gt.0.Q+00) h=x2-x
    ...
    if((x-x2)*(x2-x1).ge.0.Q+00)then
      do i=1,nvar
        ystart(i)=y(i)
      end do
      if(kmax.ne.0.Q+00)then
        kount=kount+1
        xp(kount)=x
        do i=1,nvar
          yp(i,kount)=y(i)
...
!U    USES derivs
  ...
  PARAMETER (A2=.2Q+00,A3=.3Q+00,A4=.6Q+00,A5=1.Q+00,A6=.875Q+00,B21=.2Q+00,B31=3.Q+00/40.Q+00, &
             B32=9.Q+00/40.Q+00,B41=.3Q+00,B42=-.9Q+00,B43=1.2Q+00,B51=-11.Q+00/54.Q+00,B52=2.5Q+00, &
             B53=-70.Q+00/27.Q+00,B54=35.Q+00/27.Q+00,B61=1631.Q+00/55296.Q+00,B62=175.Q+00/512.Q+00, &
             B63=575.Q+00/13824.Q+00,B64=44275.Q+00/110592.Q+00,B65=253.Q+00/4096.Q+00,C1=37.Q+00/378.Q+00, &
             C3=250.Q+00/621.Q+00,C4=125.Q+00/594.Q+00,C6=512.Q+00/1771.Q+00,DC1=C1-2825.Q+00/27648.Q+00, &
             DC3=C3-18575.Q+00/48384.Q+00,DC4=C4-13525.Q+00/55296.Q+00,DC5=-277.Q+00/14336.Q+00, &
             DC6=C6-.25Q+00)
  do i=1,n
...
!    USES derivs,rkck
  ...
  PARAMETER (SAFETY=0.9Q+00,PGROW=-.2Q+00,PSHRNK=-.25Q+00,ERRCON=1.89Q-4)
  h=htry
1 call rkck(y,dydx,n,x,h,ytemp,yerr,derivs)
  ...
  if(errmax.gt.1.)then
    ...
    h=sign(max(abs(htemp),0.1Q+00*abs(h)),h)
    xnew=x+h
