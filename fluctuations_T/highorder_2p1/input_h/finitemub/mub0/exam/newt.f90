subroutine newt(x, n, check, functionn)
  ...
  logical check
  real(16) x(n), fvec, TOLF, TOLMIN, TOLX, STPMX
  parameter (NP=40, MAXITS=200, TOLF=1.Q-04, TOLMIN=1.Q-06, TOLX=1.Q-07, STPMX=100.Q+00)

  common /newtv/ fvec(NP), nn       !Communicates with fmin
  save /newtv/
!Uses fdjac, fmin, lnsrch, lubksb, ludcmp
  integer i, its, j, indx(NP)
  real(16) d, den, f, fold, stpmax, sum, temp, test, fjac(NP,NP), g(NP), p(NP), xold(NP) !, fmin
  external ffmin, functionn

  nn=n
!  f=fmin(x)                         !The vector fvec is also computed by this call
  call ffmin(x, f, functionn)
  test=0.Q+00                        !Test for initial guess being a root. Use more stringent
  ...
  sum=0.Q+00                            !Calculate stpmax for line searches
  do i=1, n
    sum=sum+x(i)**2
  end do
...
!lnsrch returns new x and f. It also calculates fvec at the new x when it calls fmin
	...
	if(check) then                     !Check for gradient of f zero, i.e., spurious convergence
	  ...
	  den=max(f,0.5Q+00*n)
	  do i=1, n
	    temp=abs(g(i))*max(abs(x(i)),1.Q+00)/den
	...
	test=0.Q+00                            !Test for convergence on delt-x
	do i=1, n
	  temp=(abs(x(i)-xold(i)))/max(abs(x(i)),1.Q+00)
	  if(temp.gt.test)test=temp
	end do
...
subroutine fdjac(n, x, fvec, np, df, functionn)
  ...
  parameter (NMAX=40,EPS=1.Q-04)
!Uses funcv
  integer i, j
  real(16) h, temp, f(NMAX)
  external functionn
...
subroutine lnsrch(n, xold, fold, g, p, x, f, stpmax, check, func, functionn)
  ...
  parameter (ALF=1.Q-04,TOLX=1.Q-07)
  external func, functionn
!Uses func
  integer i
  real(16) a, alam, alam2, alamin, b, disc, f2, fold2, rhs1, rhs2, slope, sum, temp, &
                 test, tmplam
...
!  if(slope >= 0.0) pause 'roundoff problem in lnsrch'
  ...
  test=0.Q+00
  do i=1, n
    temp=abs(p(i))/max(abs(xold(i)),1.Q+00)
	if(temp.gt.test)test=temp
  end do
...
!	f=func(x)
	...
	if(alam.lt.alamin) then
	  ...
	  if(alam.eq.1.) then
	    tmplam=-slope/(2.Q+00*(f-fold-slope))
	  else
		...
		if(a.eq.0.) then
		  tmplam=-slope/(2.Q+00*b)
		else
		  disc=b*b-3.Q+00*a*slope
		  if(disc.lt.0.) pause 'roundoff problem in lnsrch'
		  tmplam=(-b+sqrt(disc))/(3.Q+00*a)
		end if

		if(tmplam.gt..5*alam) tmplam=5.Q-01*alam
